(function () {
  ...
  function sidebarNav(active) {
    ...
    var html = '<div class="sidebar"><div class="brand"><div class="brand-mark">' + starIcon(16, "#fff") + '</div>ScanStars</div>';
  ...
  function appShell(active, title, contentHtml) {
    return '<div class="app-shell">' + sidebarNav(active) +
      '<div class="main"><div class="topbar"><h1>' + esc(title) + '</h1></div><div class="content">' + contentHtml + '</div></div></div>';
  }
  ...
  async function viewMyNetwork() {
    ...
    var html = '<div class="tiles" style="grid-template-columns:repeat(2,minmax(0,1fr))">' + tile("Sub-partners", downline.length) + tile("Override earned", money(l2Total)) + '</div>';
  ...
  async function viewAdminTransactions() {
    ...
    var totalRevenue = shops.filter(function (s) { return s.status === "paid"; }).reduce(function (a, s) { return a + s.amount; }, 0);
    var html = '<div class="tiles" style="grid-template-columns:repeat(3,minmax(0,1fr))">' +
      tile("Total shops", shops.length) + tile("Paid & live", shops.filter(function (s) { return s.status === "paid"; }).length) + tile("Revenue collected", money(totalRevenue)) + '</div>';
    if (!shops.length) {
      html += '<div class="card muted" style="padding:32px;text-align:center">No transactions yet.</div>';
    } else {
      var rows = shops.map(function (s) {
        return '<tr><td>' + esc(s.shopName) + '</td><td class="mono">' + esc(s.agentPhone) + '</td><td class="mono">' + money(s.amount) + '</td><td>' + statusPill(s.status) + '</td><td class="mono">' + fmtDate(s.paidAt || s.createdAt) + '</td></tr>';
      }).join("");
      html += '<div class="card table-wrap"><table><thead><tr><th>Shop</th><th>Agent</th><th>Amount</th><th>Status</th><th>Date</th></tr></thead><tbody>' + rows + '</tbody></table></div>';
    }
